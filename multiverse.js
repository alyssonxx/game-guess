(() => {
  ...
  const UNIVERSES={
    ...
    dragonball:{title:'Dragon Ball',icon:'🐉',color:'#ff8b29',source:'109 personagens/variações',desc:'Do clássico ao Super, com transformações, fusões, heróis, rivais e vilões.',eyebrow:'🐉 QUEM É O PERSONAGEM?',theme:'dragonball',roundTitle:'Do clássico ao Super: reconhece quem é?',filters:{title:'🔥 Série',help:'Escolha a fase de Dragon Ball',items:{all:'Todas',classic:'Dragon Ball clássico',z:'Dragon Ball Z',gt:'Dragon Ball GT',super:'Dragon Ball Super',variants:'✨ Transformações e fusões'}}},
    yugioh:{title:'Yu-Gi-Oh!',icon:'🃏',color:'#9b6dff',source:'69 personagens/variações',desc:'Duel Monsters e GX com duelistas, vilões, formas e estilos de deck.',eyebrow:'🃏 QUEM É O DUELISTA?',theme:'yugioh',roundTitle:'É hora do duelo. Quem é?',filters:{title:'🧿 Geração',help:'Do clássico até GX',items:{all:'Todas',classic:'Clássico • Duel Monsters',gx:'Yu-Gi-Oh! GX',variants:'✨ Formas e decks especiais'}}},
    naruto:{title:'Naruto',icon:'🍥',color:'#ff9b42',source:'104 personagens/variações',desc:'Clássico e Shippuden com vilas, clãs, Akatsuki, Kages e formas especiais.',eyebrow:'🍥 QUE NINJA É ESSE?',theme:'naruto',roundTitle:'Habilidade, vila ou imagem: reconheça o ninja.',filters:{title:'🍃 Fase',help:'Escolha a fase da história',items:{all:'Todas',classic:'Naruto clássico',shippuden:'Naruto Shippuden',variants:'✨ Modos e formas especiais'}}},
    saintseiya:{title:'Cavaleiros do Zodíaco',icon:'♈',color:'#f7d66d',source:'101 personagens',desc:'Clássico, Ouro, Poseidon, Hades e The Lost Canvas em um universo próprio.',eyebrow:'♈ ELEVE O COSMO!',theme:'saintseiya',roundTitle:'Armadura, técnica, origem ou imagem: quem é?',filters:{title:'✨ Saga / grupo',help:'Escolha a era ou misture tudo',items:{all:'Todos',classic:'Clássico • Bronze e Prata',gold:'Cavaleiros de Ouro',poseidon:'Saga de Poseidon',hades:'Saga de Hades',lostcanvas:'The Lost Canvas'}}},
  ...
  const MODES={
    classic:{title:'Clássico',icon:'🧠',rounds:25,desc:'25 rodadas variadas'},
    survival:{title:'Survival',icon:'❤️',rounds:70,desc:'3 vidas'},
    blitz:{title:'Blitz',icon:'⏱️',rounds:80,desc:'2 minutos'},
    chaos:{title:'Caos',icon:'🎲',rounds:40,desc:'dificuldade e estilo mudam a cada rodada'}
  };
  const DIFF={
    easy:{title:'Fácil',icon:'🌱',attempts:6,pieces:2,mult:.85,mask:.0},
    normal:{title:'Normal',icon:'🎯',attempts:6,pieces:1,mult:1,mask:.08},
    hard:{title:'Difícil',icon:'🔥',attempts:5,pieces:0,mult:1.35,mask:.22},
  ...
  async function start(){
    ...
    try{
      ...
      session={config:{...setup},items:shuffle(d.items),index:0,wins:0,losses:0,score:0,streak:0,lives:setup.mode==='survival'?3:null,roundResolved:false,hintsUsed:0,wrong:0,revealed:0,revealOrder:[],attempts:0,current:null,activeDifficulty:setup.difficulty,roundChallenge:setup.challenge,shownClues:new Set(),blitzEnds:setup.mode==='blitz'?Date.now()+120000:null};
  ...
  function selectRoundStyle(){
    session.activeDifficulty=session.config.mode==='chaos'?pick(Object.keys(DIFF)):session.config.difficulty;
    const pool=['image','ability','origin','dossier','blind'];
    session.roundChallenge=(session.config.mode==='chaos'||session.config.challenge==='random')?pick(pool):session.config.challenge;
  }
  function maxRounds(){if(session.config.mode==='classic')return Math.min(25,session.items.length);if(session.config.mode==='chaos')return Math.min(40,session.items.length);return session.items.length;}
  function preload(url,ms=8500){return new Promise(r=>{if(!url)return r(false);const i=new Image();let done=false;const f=x=>{if(done)return;done=true;clearTimeout(t);i.onload=i.onerror=null;r(x)};const t=setTimeout(()=>f(false),ms);i.onload=()=>f(i.naturalWidth>60&&i.naturalHeight>60);i.onerror=()=>f(false);i.src=url})}

  async function loadRound(){
    if(!session)return;
    if(session.config.mode==='survival'&&session.lives<=0)return endSession('lives');
    ...
    let item=session.items[session.index],tries=0;
    while(item&&tries<4&&!(await preload(item.image))){session.index++;item=session.items[session.index];tries++;}
    if(!item)return endSession('images');
    ...
    setupImage(item.image);
    let initial=diff.pieces;
    if(['ability','origin','dossier','blind'].includes(session.roundChallenge))initial=Math.min(initial,1);
  ...
  function setupImage(url){
    const g=$('universeImageGrid');g.innerHTML='';
    for(let i=0;i<6;i++){
      const row=Math.floor(i/3),col=i%3,p=document.createElement('div');p.className='image-piece universe-piece';p.id=`u-piece-${i}`;
      const img=document.createElement('img');img.src=url;img.alt='';img.style.cssText=`width:300%;height:200%;object-fit:cover;left:${-col*100}%;top:${-row*100}%;${session?.config?.universe==='pokemon'&&session?.config?.filter==='silhouette'?'filter:brightness(0);':''}`;
      p.appendChild(img);g.appendChild(p);
    }
  ...
  function revealPiece(play=true){
    ...
    if(session.roundChallenge!=='blind')$(`u-piece-${idx}`)?.classList.add('revealed');
    $('universeImageStatus').textContent=session.roundChallenge==='blind'?'Imagem bloqueada • modo Só pistas':`${session.revealed} fragmento${session.revealed===1?'':'s'} revelado${session.revealed===1?'':'s'}`;
  ...
  function renderHints(){
    ...
    shuffle(list).forEach((c,i)=>{
      ...
      b.innerHTML=`<span>${['🧩','🔎','📌','✨','💥','🌍'][i%6]}</span>${esc(c.label.replace(/^\S+\s*/,''))}`;
  ...
  function updateUI(){
    ...
    $('universeRoundMetric').textContent=`🎯 ${session.index+1} / ${['classic','chaos'].includes(session.config.mode)?maxRounds():'∞'}`;
    $('universeLivesMetric').classList.toggle('hidden',session.config.mode!=='survival');
    if(session.config.mode==='survival')$('universeLivesMetric').textContent='❤️'.repeat(session.lives)+'🖤'.repeat(3-session.lives);
    ...
    $('universeRoundEyebrow').textContent=session.config.universe==='random'?`🎲 ${currentUniverseLabel().toUpperCase()}`:u.eyebrow;
    $('universeRoundTitle').textContent=u.roundTitle||'Observe e arrisque';
  ...
  function roundValue(){if(!session)return 0;const d=DIFF[session.activeDifficulty];const challengeBonus={image:1,ability:1.05,origin:1.05,dossier:.95,blind:1.25}[session.roundChallenge]||1;return Math.max(80,Math.round((600-session.hintsUsed*70-session.wrong*85)*d.mult*challengeBonus*combo(session.streak)))}
  ...
  function guess(){
    ...
    session.wrong++;session.attempts--;
    ...
    $('universeHintDisplay').innerHTML=`❌ <strong>${esc(val)}</strong> não é a resposta.${extra}`;
    if(session.attempts<=0)lose('tentativas');else updateUI();
  }
  function win(){
  ...
  function lose(reason='pulo'){
    if(session.roundResolved)return;session.roundResolved=true;session.losses++;session.streak=0;
    if(session.config.mode==='survival')session.lives=Math.max(0,session.lives-1);
    updateProfileLoss();
    if(session.roundChallenge!=='blind')for(let i=session.revealed;i<6;i++)revealPiece(false);
    sound('lose');
    if(session.config.mode==='blitz'){toast('❌ Passou',`Era ${session.current.name}.`,'error');session.index++;setTimeout(()=>loadRound(),520);return;}
  ...
  function showResult(won,pts){
    ...
    $('universeResultImage').src=x.image;
    ...
    $('universeRewardLine').innerHTML=won?`⭐ <b>+${pts} pontos</b> • 🔥 sequência ${session.streak}`:`A sequência foi zerada.${session.config.mode==='survival'?` Restam <b>${session.lives}</b> vida(s).`:''}`;
    $('universeResultOverlay').classList.add('active');$('universeResultOverlay').setAttribute('aria-hidden','false');
    resultAction=()=>{closeResult();session.index++;loadRound();};
  }
