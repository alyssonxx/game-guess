(() => {
  ...
  function selectFighter(id) {
    ...
    const dmRows = f.dm ? `<div class="kof-sdm-card"><span>❄️ DM / SUPER COMUM</span><div class="kof-move-row"><div><b>${esc(f.dm.name)}</b>${f.dm.note ? `<small>${esc(f.dm.note)}</small>` : ''}</div><code>${esc(f.dm.command)}</code></div></div>` : '';
    const sdmRows = f.sdm ? `<div class="kof-sdm-card"><span>🔥 SDM</span><div class="kof-move-row"><div><b>${esc(f.sdm.name)}</b>${f.sdm.note ? `<small>${esc(f.sdm.note)}</small>` : ''}</div><code>${esc(f.sdm.command)}</code></div></div>` : '';
    const hsdmRows = f.hsdm ? `<div class="kof-sdm-card"><span>💥 HSDM / MAX2</span><div class="kof-move-row"><div><b>${esc(f.hsdm.name)}</b>${f.hsdm.note ? `<small>${esc(f.hsdm.note)}</small>` : ''}</div><code>${esc(f.hsdm.command)}</code></div></div>` : '';
    detail.innerHTML = `<div class="kof-detail-hero"><div class="kof-detail-avatar ${teamSlug(f.team)}">${esc(fighterInitials(f.name))}</div><div><p class="eyebrow">${esc(f.team)}</p><h2>${esc(f.name)}</h2><span>${esc(f.style)}</span></div></div><div class="kof-move-list">${(f.moves || []).map(m => `<div class="kof-move-row"><div><b>${esc(m.name)}</b>${m.note ? `<small>${esc(m.note)}</small>` : ''}</div><code>${esc(m.command)}</code></div>`).join('')}</div>${dmRows}${sdmRows}${hsdmRows}<div class="kof-combo-card"><span>⚡ COMBO RÁPIDO</span><b>${esc(f.combo || '—')}</b></div><div class="kof-tip-card"><span>💡 COMO USAR</span><p>${esc(f.tip || 'Treine os especiais e confirme os golpes antes de gastar MAX.')}</p></div><div class="kof-notation-mini"><b>A</b> soco fraco • <b>B</b> chute fraco • <b>C</b> soco forte • <b>D</b> chute forte • <b>DM</b> super comum • <b>SDM</b> super avançado • <b>HSDM</b> hidden/MAX2</div><button class="kof-guide-pick" id="kofGuidePick" type="button">🔵 USAR ${esc(f.name)} NA GUIA AZUL + BOTÕES ESPECIAIS</button>`;
