(() => {
  ...
  function selectFighter(id) {
    ...
    const dmMode = f.dm?.macro?.inputMode === 'magic-plus-ii-shortcut' ? '⚡ ATALHO MP2' : '✅ MP2 AUDITADO';
    const dmNative = f.dm?.nativeCommand && f.dm.nativeCommand !== f.dm.command ? `<small style="display:block;margin-top:6px"><b>Alternativa nativa:</b> ${esc(f.dm.nativeCommand)}</small>` : '';
    const dmRows = f.dm ? `<div class="kof-sdm-card"><span>⚡ DM • ${dmMode}</span><div class="kof-move-row"><div><b>${esc(f.dm.name)}</b>${f.dm.note ? `<small>${esc(f.dm.note)}</small>` : ''}${dmNative}</div><code>${esc(f.dm.command)}</code></div></div>` : '';
    const sdmMode = f.sdm?.macro?.inputMode === 'magic-plus-ii-shortcut' ? '⚡ ATALHO MP2' : '✅ MP2 AUDITADO';
    const sdmNative = f.sdm?.nativeCommand && f.sdm.nativeCommand !== f.sdm.command ? `<small style="display:block;margin-top:6px"><b>Alternativa nativa:</b> ${esc(f.sdm.nativeCommand)}</small>` : '';
    const sdmRows = f.sdm ? `<div class="kof-sdm-card"><span>🔥 SDM/MAX • ${sdmMode}</span><div class="kof-move-row"><div><b>${esc(f.sdm.name)}</b>${f.sdm.note ? `<small>${esc(f.sdm.note)}</small>` : ''}${sdmNative}</div><code>${esc(f.sdm.command)}</code></div></div>` : '';
    const hsdmMode = f.hsdm?.macro?.inputMode === 'native-reviewed' ? '🎮 NATIVO REVISADO' : '⚡ ATALHO MP2';
    const nativeAlt = f.hsdm?.nativeCommand && f.hsdm.nativeCommand !== f.hsdm.command ? `<small style="display:block;margin-top:6px"><b>Alternativa nativa:</b> ${esc(f.hsdm.nativeCommand)}</small>` : '';
    const hsdmRows = f.hsdm ? `<div class="kof-sdm-card"><span>💥 HSDM/MAX2 • ${hsdmMode}</span><div class="kof-move-row"><div><b>${esc(f.hsdm.name)}</b>${f.hsdm.note ? `<small>${esc(f.hsdm.note)}</small>` : ''}${nativeAlt}</div><code>${esc(f.hsdm.command)}</code></div></div>` : '';
