...
function inject(){
  ...
  if(home){const banner=document.createElement('section');banner.className='geo-home-banner';banner.innerHTML=`<div class="geo-home-icon">🌍</div><div><p class="eyebrow">🧭 GEOGUESS ARENA</p><h2>Explore ruas reais em 360°</h2><p>Explore imagens de rua do Mapillary, avance pelas setas, procure placas e pistas, marque seu palpite no mapa e descubra a distância até o ponto correto.</p></div><div class="geo-home-actions"><button class="primary-btn" id="homeGeoButton">🌍 JOGAR GEOGUESS</button><button class="secondary-btn" id="homeGeoArenaButton">⚔️ CRIAR ARENA</button></div>`;const social=home.querySelector('.social-home-banner'),quiz=home.querySelector('.quiz-home-banner');home.insertBefore(banner,social||quiz||home.firstChild);}
  main.insertAdjacentHTML('beforeend',`
  <section class="screen geo-setup-screen" id="geoSetupScreen">
    <div class="section-heading"><button class="back-link" id="geoBack">← Voltar</button><div><p class="eyebrow">🌍 GEOGUESS ARENA</p><h2>Onde no mundo?</h2><p>Você será colocado em uma rua real. Mova-se, olhe em 360°, reúna pistas e depois marque onde acha que começou.</p></div></div>
    <div class="geo-setup-layout">
      ...
      <section class="geo-setup-card"><h3>⚔️ Arena online</h3><label>Jogadores<select id="geoMaxPlayers">${[2,3,4,5,6,7,8].map(n=>`<option value="${n}">${n} jogadores${n===2?' — 1x1':''}</option>`).join('')}</select></label><div class="geo-room-actions"><button class="primary-btn" id="geoCreateRoom">CRIAR SALA</button><div class="geo-join"><input id="geoJoinCode" maxlength="6" placeholder="ABC123"><button class="secondary-btn" id="geoJoinRoom">ENTRAR</button></div></div><small>Todos começam na mesma imagem/sequência do Mapillary e podem se movimentar livremente. Tempo: 60 segundos por rodada.</small></section>
  ...
  <section class="screen geo-game-screen" id="geoGameScreen">
    ...
    <div class="geo-stage" id="geoStage">
      <section class="geo-street-card">
        ...
        <div class="geo-street-toolbar"><button id="geoReturnStart" class="geo-float-btn" title="Voltar ao ponto inicial">↩️ INÍCIO</button><span id="geoMoveHint">🚶 Mova-se pelas ruas para encontrar pistas</span></div>
      </section>
...
async function ensureViewer(){
  ...
  viewer=new mly.Viewer({accessToken:mapillaryToken,container:'geoStreetView',fallback:{image:true,navigation:true}});
...
async function displayRound(){
  ...
  $('geoTimerLabel').classList.toggle('hidden',mode==='solo');$('geoMoveHint').textContent='🚶 Mova-se pelas ruas, gire a câmera e procure pistas';$('geoFeedback').textContent='Clique no mapa para colocar seu palpite.';$('geoSubmitGuess').classList.remove('hidden');$('geoSubmitGuess').disabled=true;$('geoNextRound').classList.add('hidden');$('geoReturnStart').disabled=false;selected=null;
...
window.GameGuessGeo={open};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind);else bind();
})();
