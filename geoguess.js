...
function scoreDistance(km){const scoreConfig={distance:km,formula:'distanceBased'};const pts=window.GameGuessScoring?.calculateScore?.('geoguess',scoreConfig);return pts||Math.max(0,Math.round(5000*Math.exp(-km/1800)));}
...
function nextSolo(){if(!solo)return;if(solo.index>=solo.questions.length-1){const p=CORE()?.getProfile?.()||{};p.gamesPlayed=Number(p.gamesPlayed||0)+1;p.gamesWon=Number(p.gamesWon||0)+1;p.geoPlayed=Number(p.geoPlayed||0)+1;p.geoBestScore=Math.max(Number(p.geoBestScore||0),solo.score);p.highScore=Math.max(Number(p.highScore||0),solo.score);const coinsEarned=window.GameGuessScoring?.pointsToCoinReward?.(solo.score,'normal')||4;p.coins=Number(p.coins||0)+coinsEarned;window.GameGuessRanked?.record?.(p,{kind:'geoguess',score:solo.score,mode:'solo',universe:'geoguess',challenge:config.region,difficulty:'mapillary-street',correct:solo.questions.length,wrong:0,won:true});CORE()?.replaceProfile?.(p);CORE()?.saveProfile?.();FB()?.syncLocalProfile?.(p);CORE()?.spawnConfetti?.();toast('GeoGuess concluído',`${solo.score.toLocaleString('pt-BR')} / ${(solo.questions.length*5000).toLocaleString('pt-BR')} pontos • +${coinsEarned} moedas`);solo=null;return show('geoSetupScreen');}solo.index++;solo.answered=false;displayRound();}
...
async function createRoom(){if(!user())return toast('Login necessário','Entre na sua conta para criar uma Arena GeoGuess.','error');config.region=$('geoRegion').value;config.difficulty=$('geoArenaDifficulty').value||'normal';config.rounds=Number($('geoRounds').value)||5;config.maxPlayers=Number($('geoMaxPlayers').value)||2;const b=$('geoCreateRoom');b.disabled=true;b.textContent='PREPARANDO MAPILLARY...';try{const questions=await fetchRounds();const diff=GEO_DIFFICULTIES[config.difficulty]||GEO_DIFFICULTIES.normal;roomCode=await FB().createGeoRoom({questions,region:config.region,difficulty:config.difficulty,maxPlayers:config.maxPlayers,timerSec:diff.timerSec});localStorage.setItem('gameGuessLastGeoRoom',roomCode);mode='arena';watchRoom(roomCode);$('geoWaiting').classList.remove('hidden');toast('Sala criada',`Código ${roomCode}`)}catch(e){toast('Erro ao criar sala',e.message||String(e),'error')}finally{b.disabled=false;b.textContent='CRIAR SALA';}}
...
async function advanceArena(idx){if(!room||room.status!=='playing'||Number(room.roundIndex)!==idx||room.hostUid!==user()?.uid)return;try{await FB().mutateGeoRoom(roomCode,(r)=>{if(Number(r.roundIndex)!==idx)return r;const total=Number(r.config?.rounds||r.questions?.length||5);if(idx>=total-1){r.status='finished';r.roundState='finished';r.finishedAt=Date.now();r.expiresAt=Date.now()+2*60*60*1000;const list=Object.values(r.players||{}).sort((a,b)=>Number(b.score||0)-Number(a.score||0));r.winnerUid=list[0]?.uid||'';return r;}const diff=GEO_DIFFICULTIES[r.config?.difficulty||'normal']||GEO_DIFFICULTIES.normal;r.roundIndex=idx+1;r.roundDeadline=Date.now()+diff.timerSec*1000;r.roundState='playing';for(const p of Object.values(r.players||{})){p.roundScore=0;p.distanceKm=0;p.guessLat=null;p.guessLng=null;p.steps=0;}return r;});}catch{}}
