...
async function probe(url,timeout=3500){
  ...
  try{
    const r=await fetch(url,{method:'GET',redirect:'follow',cache:'no-store',signal:ctrl.signal,headers:{'User-Agent':'GameGuess-V14.2-Health/1.0'}});
...
export default async function handler(req,res){
  ...
  res.status(200).json({ok:services.emulator.ok&&services.netplay.ok,services,turnConfigured:Boolean(process.env.KOF_TURN_URL),checkedAt:Date.now(),version:'14.2.0'});
}
