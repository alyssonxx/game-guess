...
function pickImage(items){
  ...
  const hasSequence=x=>Boolean(x?.sequence);
  ...
  const pool=sphericalSequence.length?sphericalSequence:spherical.length?spherical:sequenced.length?sequenced:usable;
...
async function mapillaryImages(token,lat,lng){
  ...
  try{
    ...
    u.searchParams.set('bbox',bboxAround(lat,lng,5));
...
export default async function handler(req,res){
  ...
  if(pool.length<count*3)pool=region==='world'?LOCATIONS:[...pool,...shuffle(LOCATIONS.filter(s=>s[0]!==region))];
  const seeds=shuffle(pool).slice(0,Math.min(pool.length,Math.max(count*4,12)));
  const candidates=[];
  ...
  if(candidates.length<count)return out(res,503,{error:`O Mapillary encontrou apenas ${candidates.length} de ${count} locais navegáveis nesta tentativa. Tente novamente ou escolha outra região.`,provider:'mapillary',candidates});
  return out(res,200,{provider:'mapillary',candidates:shuffle(candidates).slice(0,count)});
}
