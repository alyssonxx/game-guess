...
function q(category,question,correct,wrong,difficulty='normal',source='Gerador Game Guess'){
  const opts=shuffle([String(correct),...wrong.map(String)]).slice(0,4);
  if(!opts.includes(String(correct)))opts[0]=String(correct);
  return {id:`${source.startsWith('Tryvia')?'ext':'gen'}:${category}:${hash(question+'|'+correct+'|'+Math.random())}`,category,question:String(question),correct:String(correct),options:shuffle([...new Set(opts)]).slice(0,4),difficulty,source};
}
...
function qNum(cat,prompt,correct,spread=10,diff='normal'){const opts=numericOptions(correct,spread);return {id:`gen:${cat}:${hash(prompt+'|'+correct+'|'+Math.random())}`,category:cat,question:prompt,correct:String(correct),options:opts,difficulty:diff,source:'Gerador Game Guess'};}
...
function generated(category,difficulty='normal'){
  ...
  if(category==='geografia'){
    ...
    if(inverse){const wrong=shuffle(CAPITALS.filter(x=>x[0]!==country).map(x=>x[0])).slice(0,3);return q('geografia',`Qual país tem ${capital} como capital?`,country,wrong,difficulty);}
    const wrong=shuffle(CAPITALS.filter(x=>x[0]!==country).map(x=>x[1])).slice(0,3);return q('geografia',pick([`Qual é a capital de ${country}?`,`A cidade que funciona como capital de ${country} é:`,`Entre as opções, qual é a capital de ${country}?`]),capital,wrong,difficulty);
  }
  if(category==='historia'){
    const [event,year]=pick(EVENTS),inverse=Math.random()<.35;
    if(inverse){const wrong=shuffle(EVENTS.filter(x=>x[0]!==event).map(x=>x[0])).slice(0,3);return q('historia',`Qual acontecimento desta lista está associado ao ano de ${year}?`,event,wrong,difficulty);}
    const wrong=shuffle(EVENTS.filter(x=>x[1]!==year).map(x=>x[1])).slice(0,3);return q('historia',pick([`Em que ano ocorreu: ${event}?`,`Qual ano corresponde a “${event}”?`,`Quando aconteceu “${event}”?`]),year,wrong,difficulty);
  }
  if(category==='ciencias'){
    const [name,symbol]=pick(ELEMENTS),inverse=Math.random()<.45;
    if(inverse){const wrong=shuffle(ELEMENTS.filter(x=>x[0]!==name).map(x=>x[0])).slice(0,3);return q('ciencias',`Qual elemento químico é representado pelo símbolo ${symbol}?`,name,wrong,difficulty);}
    const wrong=shuffle(ELEMENTS.filter(x=>x[1]!==symbol).map(x=>x[1])).slice(0,3);return q('ciencias',pick([`Qual é o símbolo químico do elemento ${name}?`,`Que símbolo representa o elemento ${name} na tabela periódica?`]),symbol,wrong,difficulty);
  }
  if(category==='tecnologia'){
    const [term,use]=pick(TECH),inverse=Math.random()<.4;
    if(inverse){const wrong=shuffle(TECH.filter(x=>x[0]!==term).map(x=>x[0])).slice(0,3);return q('tecnologia',`Qual tecnologia ou componente está mais associado a “${use}”?`,term,wrong,difficulty);}
    const wrong=shuffle(TECH.filter(x=>x[0]!==term).map(x=>x[1])).slice(0,3);return q('tecnologia',pick([`Qual destas opções descreve melhor a função de ${term}?`,`No contexto de tecnologia, para que serve principalmente ${term}?`]),use,wrong,difficulty);
  }
  if(category==='portugues'){
    const [word,answer,type]=pick(PORTUGUESE),wrong=shuffle(PORTUGUESE.filter(x=>x[2]===type&&x[1]!==answer).map(x=>x[1])).slice(0,3);
    return q('portugues',pick([`Qual é um ${type} de “${word}”?`,`Na língua portuguesa, qual opção funciona como ${type} de “${word}”?`,`Escolha o ${type} mais adequado para “${word}”.`]),answer,wrong,difficulty);
  }
  if(category==='musica'){
    const [inst,n]=pick(MUSIC),wrong=numericOptions(n,6).filter(x=>x!==String(n)).slice(0,3);
    return q('musica',pick([`Quantas cordas/teclas possui normalmente um ${inst}?`,`Em sua configuração tradicional, quantas cordas/teclas tem um ${inst}?`,`Qual número corresponde à quantidade tradicional de cordas/teclas de um ${inst}?`]),n,wrong,difficulty);
  }
  if(category==='esportes'){
    const [sport,detail,n]=pick(SPORT),wrong=numericOptions(n,5).filter(x=>x!==String(n)).slice(0,3);
    return q('esportes',pick([`No ${sport}, quantos ${detail}?`,`Pensando nas regras do ${sport}, qual é o número de ${detail}?`,`Qual quantidade corresponde a ${detail} no ${sport}?`]),n,wrong,difficulty);
  }
  if(category==='filmes'){
    const [title,character]=pick(FILMS),inverse=Math.random()<.45;
    if(inverse){const wrong=shuffle(FILMS.filter(x=>x[0]!==title).map(x=>x[0])).slice(0,3);return q('filmes',`Em qual filme ou franquia desta lista o personagem ${character} é uma figura central?`,title,wrong,difficulty);}
    const wrong=shuffle(FILMS.filter(x=>x[1]!==character).map(x=>x[1])).slice(0,3);return q('filmes',pick([`Qual personagem é o protagonista mais associado a “${title}”?`,`Em “${title}”, qual personagem desta lista é central à história?`]),character,wrong,difficulty);
  }
  if(category==='games'){
    const [title,character]=pick(GAMES),inverse=Math.random()<.45;
    if(inverse){const wrong=shuffle(GAMES.filter(x=>x[0]!==title).map(x=>x[0])).slice(0,3);return q('games',`A qual série de jogos o personagem ${character} está principalmente associado?`,title,wrong,difficulty);}
    const wrong=shuffle(GAMES.filter(x=>x[1]!==character).map(x=>x[1])).slice(0,3);return q('games',pick([`Qual personagem é protagonista ou ícone principal da série “${title}”?`,`Quem é uma figura central da franquia de jogos “${title}”?`]),character,wrong,difficulty);
  }
  if(category==='desenhos'){
    const [title,character]=pick(DRAWINGS),inverse=Math.random()<.45;
    if(inverse){const wrong=shuffle(DRAWINGS.filter(x=>x[0]!==title).map(x=>x[0])).slice(0,3);return q('desenhos',`Em qual desenho ou anime desta lista aparece ${character} como figura central?`,title,wrong,difficulty);}
    const wrong=shuffle(DRAWINGS.filter(x=>x[1]!==character).map(x=>x[1])).slice(0,3);return q('desenhos',pick([`Qual personagem é protagonista ou figura central de “${title}”?`,`Em “${title}”, qual personagem desta lista ocupa papel central?`]),character,wrong,difficulty);
  }
...
async function getJson(url,timeout=7000){const c=new AbortController(),t=setTimeout(()=>c.abort(),timeout);try{const r=await fetch(url,{signal:c.signal,headers:{Accept:'application/json','User-Agent':'GameGuess-Quiz/13.1'}});if(!r.ok)throw new Error(`HTTP ${r.status}`);return await r.json();}finally{clearTimeout(t)}}
...
async function externalQuestions(category,difficulty,count){
  try{
    ...
    return data.results.map((x,i)=>{
      ...
      return {id:`tryvia:${hash(question+'|'+correct)}`,category:inferred,question,correct,options:shuffle([correct,...wrong]).slice(0,4),difficulty:x.difficulty||difficulty,source:'Tryvia API'};
    }).filter(Boolean);
  }catch{return []}
}
function localFallbackQuestions(category,difficulty){
  ...
  return shuffle(pool).map(x=>({id:`local:${x.id}`,category:x.category,question:x.question,correct:String(x.correct),options:shuffle([...(x.options||[])]),difficulty,source:'Banco local Game Guess'}));
}
function fingerprint(x){return hash(norm(x.question)+'|'+norm(x.correct));}
...
export default async function handler(req,res){
  ...
  const seen=new Set((Array.isArray(body.seen)?body.seen:[]).slice(-1500).map(String));
  ...
  // Se o usuário já esgotou o histórico recente, reutiliza primeiro itens antigos do fallback,
  // sem repetir dentro da mesma partida. A API externa volta a renovar o conjunto nas próximas sessões.
  if(result.length<count){
    for(const item of localFallbackQuestions(category,difficulty)){
      const fp=fingerprint(item);if(used.has(fp))continue;used.add(fp);result.push(item);if(result.length>=count)break;
    }
  }
  guard=0;while(result.length<count&&guard++<count*80){const cat=category==='random'?pick(['geral','historia','geografia','matematica','portugues','ciencias','filmes','games','desenhos','esportes','tecnologia','musica']):category;const item=generated(cat,difficulty),fp=fingerprint(item);if(used.has(fp))continue;used.add(fp);result.push(item);}
