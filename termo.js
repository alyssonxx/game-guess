(() => {
  ...
  function endRound(won){releaseNativeKeyboard();state.ended=true;state.won=won;const cfg=modeCfg(),p=readProfile();p.termPlayed=Number(p.termPlayed||0)+1;p.termWins=Number(p.termWins||0)+(won?1:0);p.termCurrentStreak=won?Number(p.termCurrentStreak||0)+1:0;p.termBestStreak=Math.max(Number(p.termBestStreak||0),Number(p.termCurrentStreak||0));p.termModeWins={...(p.termModeWins||{})};if(won)p.termModeWins[state.mode]=Number(p.termModeWins[state.mode]||0)+1;let pts=0;if(won){pts=Math.max(80,cfg.scoreBase-(state.guesses.length-1)*28);state.sessionScore+=pts;p.coins=Number(p.coins||0)+Math.max(3,Math.min(8,Math.round(pts/120)));p.highScore=Math.max(Number(p.highScore||0),state.sessionScore);CORE()?.playSound?.('win');CORE()?.spawnConfetti?.();message(`✅ ${cfg.title} concluído em ${state.guesses.length}/${cfg.attempts} tentativas • +${pts} pontos`,'success');}else{p.coins=Number(p.coins||0)+1;CORE()?.playSound?.('lose');message(`💥 GAME OVER • Limite de ${cfg.attempts} tentativas atingido. Respostas: ${state.targets.map(x=>x.toUpperCase()).join(' • ')}`,'error');}if(Number(p.termWins||0)>=10)unlock(p,'wordsmith','🔤 Mestre das Palavras',20);if(Number(p.termBestStreak||0)>=5)unlock(p,'termoStreak','♾️ Sequência Infinita',30);window.GameGuessRanked?.record?.(p,{kind:'termo',score:pts,mode:state.mode,universe:'termo',challenge:'palavra',difficulty:'termo',correct:won?1:0,wrong:won?Math.max(0,state.guesses.length-1):state.guesses.length,won});saveProfile(p);render();}
