...
function inject(){
 ...
 const top=document.querySelector('.top-actions');if(top){const b=document.createElement('button');b.className='mini-pill';b.id='profileButton';b.title='Meu perfil';b.innerHTML='🧑 <b>Perfil</b>';top.insertBefore(b,$('achievementsButton'));}
 const home=$('homeScreen');if(home){const banner=document.createElement('section');banner.className='social-home-banner';banner.innerHTML=`<div class="social-home-avatar" id="homeAvatarPreview">🎮</div><div><p class="eyebrow">👤 SEU JOGADOR</p><h2>Perfil e estatísticas</h2><p>O sistema antigo de avatar foi retirado. Agora o foco é perfil, recordes e progresso geral.</p></div><div class="social-home-actions"><button class="primary-btn" id="homeProfileButton">ABRIR PERFIL</button></div>`;const q=home.querySelector('.quiz-home-banner');home.insertBefore(banner,q||home.firstChild);}
 main.insertAdjacentHTML('beforeend',`<section class="screen profile-screen" id="profileScreen"><div class="section-heading"><button class="back-link" id="profileBack">← Voltar</button><div><p class="eyebrow">👤 PERFIL DO JOGADOR</p><h2>Seu espaço no Game Guess</h2><p>Identidade, recordes e progresso da conta.</p></div></div><div class="profile-v15-layout"><section class="profile-v15-hero"><div id="profileAvatarHero" class="profile-avatar-hero"></div><div class="profile-v15-id"><span class="online-dot">● ONLINE</span><h2 id="profileNickLabel">Jogador</h2><p id="profileBioLabel"></p><div class="profile-favorite">🎮 Favorito: <b id="profileFavLabel">Game Guess</b></div></div></section><section class="profile-edit-card"><h3>✏️ Identidade</h3><label>Nickname<input id="profileNickInput" maxlength="22" placeholder="Seu nickname"></label><label>Bio<textarea id="profileBioInput" maxlength="120"></textarea></label><label>Jogo favorito<select id="profileFavoriteInput"><option>Game Guess</option><option>Quiz Mundial</option><option>Termo Arcade</option><option>KOF 2002 Magic Plus II</option><option>Multiverso</option><option>Arena Online</option></select></label><button class="primary-btn" id="profileSave">SALVAR PERFIL</button></section><section class="profile-stat-grid" id="profileStatGrid"></section><section class="profile-showcase"><div><h3>🏆 Destaques</h3><div id="profileHighlights" class="profile-highlight-grid"></div></div><div><h3>📊 Resumo</h3><div id="profileEquipped" class="profile-equipped"></div></div></section></div></section>`);
}
...
function stats(p){const played=Number(p.gamesPlayed||0),wins=Number(p.gamesWon||0);return [['⭐','Pontos',p.coins||0],['🏆','Maior pontuação',p.highScore||0],['🔥','Melhor sequência',p.bestStreak||0],['🎮','Partidas',played],['✅','Vitórias',wins],['🎯','Aproveitamento',played?`${Math.round(wins/played*100)}%`:'0%'],['⚔️','Vitórias Arena',p.duelWins||0]];}
function renderProfile(){const p=defaults(read()),u=FB()?.getUser?.();if($('profileAvatarHero'))$('profileAvatarHero').innerHTML='<div style="display:grid;place-items:center;width:100%;height:100%;font-size:56px">🎮</div>';$('profileNickLabel').textContent=p.nickname||u?.displayName||u?.email?.split('@')[0]||'Jogador';$('profileBioLabel').textContent=p.bio||'Seu progresso fica salvo na conta.';$('profileFavLabel').textContent=p.favoriteGame;$('profileNickInput').value=p.nickname||'';$('profileBioInput').value=p.bio;$('profileFavoriteInput').value=p.favoriteGame;$('profileStatGrid').innerHTML=stats(p).map(([i,l,v])=>`<div><span>${i}</span><small>${l}</small><b>${v}</b></div>`).join('');const level=Math.max(1,Math.floor((Number(p.gamesPlayed||0)+Number(p.gamesWon||0)*2)/10)+1);$('profileHighlights').innerHTML=`<div>🎖️ <b>Nível ${level}</b><span>Experiência geral</span></div><div>🧠 <b>${p.termBestStreak||p.bestStreak||0}</b><span>Sequência destaque</span></div><div>🥊 <b>${p.kofRating||1000}</b><span>Elo KOF</span></div><div>🧠 <b>${p.quizBestScore||0}</b><span>Melhor Quiz</span></div>`;$('profileEquipped').innerHTML=`<span title="Status geral">📈 <small>Partidas: ${Number(p.gamesPlayed||0)}</small></span><span title="Vitórias">🏆 <small>Vitórias: ${Number(p.gamesWon||0)}</small></span><span title="Modo favorito">🎮 <small>Favorito: ${esc(p.favoriteGame||'Game Guess')}</small></span>`;}
...
function openAvatar(){CORE()?.toast?.('Avatar removido','O sistema antigo de avatar foi retirado nesta versão.','info')}
function bind(){
 inject();const p=defaults(read());write(p);const h=$('homeAvatarPreview');if(h)h.textContent='🎮';
 $('profileButton')?.addEventListener('click',openProfile);$('homeProfileButton')?.addEventListener('click',openProfile);$('profileBack')?.addEventListener('click',()=>CORE()?.showScreen?.('homeScreen'));
 ...
 window.addEventListener('gameguess:authchange',()=>{const h=$('homeAvatarPreview');if(h)h.textContent='🎮';if($('profileScreen')?.classList.contains('active'))renderProfile()});
}
